...
def run_smoke_test():
    ...
    c = snap.commute
    failures += 0 if check("commute.timestamp is a datetime", isinstance(c.timestamp, datetime)) else 1
    failures += 0 if check("optempo level in 1–5", 1 <= c.optempo.level <= 5,
                            f"got {c.optempo.level}") else 1
    ...
    failures += 0 if check("at least 1 corridor", len(c.corridors) >= 1,
                            f"got {len(c.corridors)}") else 1
    for corridor in c.corridors:
        failures += 0 if check(
            f"corridor '{corridor.id}' speed_ratio > 0",
            corridor.speedRatio > 0, f"got {corridor.speedRatio}"
        ) else 1

    d = snap.doomsday
    failures += 0 if check("doomsday.timestamp is a datetime", isinstance(d.timestamp, datetime)) else 1
    failures += 0 if check("at least 1 market", len(d.markets) >= 1,
                            f"got {len(d.markets)}") else 1
    for mkt in d.markets:
        failures += 0 if check(
            f"market '{mkt.slug}' price in [0, 1]",
            0.0 <= mkt.price <= 1.0, f"got {mkt.price}"
        ) else 1
        failures += 0 if check(
            f"market '{mkt.slug}' has a region",
            bool(mkt.region)
        ) else 1
    ...
    commute_rows = con.execute("SELECT COUNT(*) FROM commute_snapshots").fetchone()[0]
    failures += 0 if check("commute_snapshots has 1 row", commute_rows == 1,
                            f"got {commute_rows}") else 1

    corridor_rows = con.execute("SELECT COUNT(*) FROM corridor_snapshots").fetchone()[0]
    failures += 0 if check(
    ...
    market_rows = con.execute("SELECT COUNT(*) FROM market_snapshots").fetchone()[0]
    failures += 0 if check(
    ...
    # -----------------------------------------------------------------------
    row = con.execute(
        "SELECT optempo_level, optempo_label, is_anomalous FROM commute_snapshots"
    ).fetchone()
    failures += 0 if check("optempo_level persisted correctly",
                            row[0] == c.optempo.level, f"got {row[0]}") else 1
    failures += 0 if check("optempo_label persisted correctly",
                            row[1] == c.optempo.label, f"got {row[1]}") else 1
    failures += 0 if check("is_anomalous persisted correctly",
                            row[2] == c.summary.isAnomalous, f"got {row[2]}") else 1

    null_prices = con.execute(
        "SELECT COUNT(*) FROM market_snapshots WHERE price IS NULL"
    ).fetchone()[0]
    failures += 0 if check("no NULL prices in market_snapshots", null_prices == 0,
                            f"got {null_prices}") else 1

    null_ids = con.execute(
        "SELECT COUNT(*) FROM corridor_snapshots WHERE corridor_id IS NULL"
    ).fetchone()[0]
    failures += 0 if check("no NULL corridor_ids", null_ids == 0,
                            f"got {null_ids}") else 1
